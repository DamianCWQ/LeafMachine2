...
try:
    # prefer stdlib importlib.resources where possible
...
def get_config_file(config_path):
    ...
    if _importlib_files is not None:
        try:
            pkg_files = _importlib_files("detectron2.model_zoo")
            cfg_file = str(pkg_files.joinpath("configs").joinpath(config_path))
        except Exception:
